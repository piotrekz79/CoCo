...
public class Flow {
	...
	private String pushMplsLabel = null;
	private String dstMAC= null;
	private boolean stripvlan = false;
	private boolean popMplsLabel = false;
	...
	public void popMplsLabel() {
		this.popMplsLabel = true;
	}
	...
	public String buildFlow() {
		...
		if (matchMplsLabel != null) {
			...
			flow += String.format("<mpls-label>%s</mpls-label>", matchMplsLabel);
			flow += "</protocol-match-fields>";
		}
		if (matchDstIpv4Prefix != null) {
			flow += String.format("<ipv4-destination>%s</ipv4-destination>", matchDstIpv4Prefix);
		}
		flow += "</match>";
		...
		int order = 0;
		if (stripvlan) {
			flow += String.format("<action><order>%d</order><pop-vlan-action/></action>", order);
			order++;
		}
		if (pushVlanId != null) {
			flow += String.format("<action><order>%d</order><push-vlan-action>", order);
			order++;
			flow += String.format("<ethernet-type>%s</ethernet-type>", String.valueOf(0x8100));
			flow += "</push-vlan-action></action>";
			flow += String.format("<action><order>%d</order><set-field><vlan-match><vlan-id>", order);
			order++;
			flow += "<vlan-id-present>true</vlan-id-present>";
		...
		if (pushMplsLabel != null) {
			flow += String.format("<action><order>%d</order><push-mpls-action>", order);
			order++;
			flow += String.format("<ethernet-type>%s</ethernet-type>", String.valueOf(0x8847));
			flow += "</push-mpls-action></action>";
			flow += String.format("<action><order>%d</order><set-field><protocol-match-fields>", order);
			order++;
			flow += String.format("<mpls-label>%s</mpls-label>", pushMplsLabel);
			flow += "</protocol-match-fields></set-field></action>";
		}
		if (popMplsLabel) {
			flow += String.format("<action><order>%d</order><pop-mpls-action>", order);
			flow += String.format("<ethernet-type>%s</ethernet-type>", String.valueOf(0x0800));
			flow += "</pop-mpls-action></action>";
			order++;
		}
		if (dstMAC != null) {
			flow += String.format("<action><order>%d</order><set-field><ethernet-match><ethernet-destination>", order);
			order++;
			flow += String.format("<address>%s</address>", dstMAC);
			flow += "</ethernet-destination></ethernet-match></set-field></action>";
		}
		if (outport != null) {
			flow += String.format("<action><order>%d</order><output-action>", order);
			order++;
			flow += String.format("<output-node-connector>%s</output-node-connector>", outport);
